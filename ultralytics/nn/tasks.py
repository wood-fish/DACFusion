...
from ultralytics.nn.modules import (
    ...
    SCDown,
    C2fCIB,
RepNCSPELAN4, ADown, SPPELAN,ResNetLayer
)
...
class BaseModel(nn.Module):
    ...
    def forward(self, x,x_ir = None, *args, **kwargs):
        """
        ...
        return self.predict(x, x_ir=x_ir, *args, **kwargs)
        #return self.predict(x, x_ir=None, *args, **kwargs)

    def predict(self, x,x_ir=None, profile=False, visualize=False, augment=False, embed=None):
        """
        ...
        """
        if augment:
            return self._predict_augment(x,x_ir)
        return self._predict_once(x, x_ir,profile, visualize)

    def _predict_once(self, x, x_ir, profile=False, visualize=False):
        """
        Perform a forward pass through the network.
        Args:
    ...
    def _predict_augment(self, x,x_ir=None):
        ...
        return self._predict_once(x,x_ir)
    ...
    def loss(self, batch, preds=None):
        ...
        if preds is None:
            if "ir" in batch:
                preds = self.forward(batch["img"], batch["ir"])
            else:
                print("--------No ir_input--------")
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Define model
        ch = self.yaml["ch"] = self.yaml.get("ch", ch)  # input channels (double for RGB + IR)
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            forward = lambda x_rgb, x_ir: self.forward(x_rgb, x_ir)[0] if isinstance(m, (
    ...
    def _predict_augment(self, x,x_ir=None):
        ...
        for si, fi in zip(s, f):
            ...
            yi = super().predict(xi,xi_ir)[0]  # forward
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        m_, args, c2, t ,is_backbone= process_layer(d, f, n, m, args, depth, width, ch_rgb, max_channels, nc, is_backbone, verbose,
                                        i, c2)
        save.extend(x % (i + 4 if is_backbone else i) for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist
        layers.append(m_)
        if i == 0:
...
def process_layer(d, f, n, m, args, depth, width, ch, max_channels, nc, is_backbone, verbose, i, c2):
    try:
        if m == 'node_mode':
            m = d[m]
            if len(args) > 0:
    ...
    if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
             BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, PSA, SCDown, C2fCIB,RepNCSPELAN4, ADown, SPPELAN, C2fAttn):
        if args[0] == 'head_channel':
            args[0] = d[args[0]]
        c1, c2 = ch[f], args[0]
        ...
        if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,C2fCIB):
        ...
        if m is HGBlock:
            ...
            n = 1
    elif m in (Detect, Pose, v10Detect):
        args.append([ch[x] for x in f])
    elif m is nn.BatchNorm2d:
        args = [ch[f]]
    elif m is Concat:
        c2 = sum(ch[x] for x in f)
    elif m in (Detect, Segment, Pose, v10Detect):
        args.append([ch[x] for x in f])
        if m is Segment:
    ...
    return m_, args, c2, t, is_backbone
