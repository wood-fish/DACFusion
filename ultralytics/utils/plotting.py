...
def plot_images(
    ...
    """Plot image grid with labels for both RGB and IR images."""
    def preprocess_tensor(tensor):
        if isinstance(tensor, torch.Tensor):
            return tensor.cpu().float().numpy()
        return tensor

    images = preprocess_tensor(images)
    ir_images = preprocess_tensor(ir_images)
    cls = preprocess_tensor(cls)
    bboxes = preprocess_tensor(bboxes)
    masks = preprocess_tensor(masks).astype(int)
    kpts = preprocess_tensor(kpts)
    batch_idx = preprocess_tensor(batch_idx)
    ...
    if np.max(ir_images[0]) <= 1:
        ir_images *= 255  # de-normalise (optional)

    # Build Image
    mosaic = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init
    ir_mosaic = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        mosaic[y : y + h, x : x + w, :] = images[i].transpose(1, 2, 0)
        ir_mosaic[y : y + h, x : x + w, :] = ir_images[i].transpose(1, 2, 0)
    ...
    if scale < 1:
        ...
        ir_mosaic = cv2.resize(ir_mosaic, tuple(int(x * ns) for x in (w, h)))

    # Annotate
    fs = int((h + w) * ns * 0.01)  # font size
    annotator = Annotator(mosaic, line_width=round(fs / 10), font_size=fs, pil=True, example=names)
    ir_annotator = Annotator(ir_mosaic, line_width=round(fs / 10), font_size=fs, pil=True, example=names)

    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
        ir_annotator.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
        if paths:
            annotator.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames
            ir_annotator.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames
        if len(cls) > 0:
            idx = batch_idx == i
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > conf_thres:
                        ...
                        ir_annotator.box_label(box, label, color=color, rotated=is_obb)
                ...
                for c in classes:
                    ...
                    ir_annotator.text((x, y), f"{c}", txt_color=color, box_style=True)
            ...
            if len(masks):
                ...
                ir_im = np.asarray(ir_annotator.im).copy()
                for j in range(len(image_masks)):
                    if labels or conf[j] > conf_thres:
                        ...
                        with contextlib.suppress(Exception):
                            ...
                            ir_im[y : y + h, x : x + w, :][mask] = (
                                ir_im[y : y + h, x : x + w, :][mask] * 0.4 + np.array(color) * 0.6
                            )
                annotator.fromarray(im)
                ir_annotator.fromarray(ir_im)

        # Convert annotated images to numpy arrays
    rgb_image = np.asarray(annotator.im)
    ir_image = np.asarray(ir_annotator.im)

    # Concatenate RGB and IR images
    combined_image = np.concatenate((rgb_image, ir_image), axis=1)

    if not save:
        return combined_image
    annotator.im.save(fname)  # save
    fname_ir = str(fname).replace('.jpg', '_ir.jpg')
    ir_annotator.im.save(fname_ir)
    if on_plot:
        on_plot(fname)
