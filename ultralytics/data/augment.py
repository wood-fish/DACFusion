...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels):
        """Create a 2x2 image mosaic for both RGB and IR images."""
        mosaic_labels = []
        s = self.imgsz
        ...
        for i in range(4):
            ...
            # Load images
            img = labels_patch["img"]
            ir_img = labels_patch["ir"]
            h, w = labels_patch.pop("resized_shape")

            # Place images in img4 and ir_img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 0, dtype=np.uint8)  # base image with 4 tiles
                ir_img4 = np.full((s * 2, s * 2, ir_img.shape[2]), 0, dtype=np.uint8)  # base IR image with 4 tiles
                # img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                # ir_img4 = np.full((s * 2, s * 2, ir_img.shape[2]), 114, dtype=np.uint8)  # base IR image with 4 tiles
            ...
            ir_img4[y1a:y2a, x1a:x2a] = ir_img[y1b:y2b, x1b:x2b]  # ir_img4[ymin:ymax, xmin:xmax]
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        final_labels = {
            ...
            "ir_file": mosaic_labels[0]["ir_file"],
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        if labels.get("ir",None) is not None:
            labels["ir"] = (labels["ir"] * r + labels2["ir"] * (1 - r)).astype(np.uint8)
...
class RandomPerspective:
    ...
    def affine_transform(self, img, border, ir_img=None):
        """
        Applies a sequence of affine transformations centered around the image center for both RGB and IR images.
        ...
        Returns:
            img (ndarray): Transformed RGB image.
            ir_img (ndarray): Transformed IR image.
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                if ir_img is not None:
                    ir_img = cv2.warpPerspective(ir_img, M, dsize=self.size, borderValue=(0, 0, 0))
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(0, 0, 0))
                # img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                if ir_img is not None:
                    ir_img = cv2.warpAffine(ir_img, M[:2], dsize=self.size, borderValue=(0, 0, 0))

        return img, ir_img, M, s
    ...
    def __call__(self, labels):
        """
        Affine images and targets for both RGB and IR modalities.

        Args:
            labels (dict): a dict of `bboxes`, `segments`, `keypoints`, `img`, `ir`.
        """
        ...
        # Affine transformation for RGB image
        img, ir, M, scale = self.affine_transform(img, border,ir)
        ...
        labels["cls"] = cls[i]
        labels["img"] = img
        # Affine transformation for IR image (if present)
        if ir is not None:
            labels["ir"] = ir
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            def apply_hsv_augmentation(image):
            ...
            apply_hsv_augmentation(img)
            if "ir" in labels:
                apply_hsv_augmentation(ir_img)
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        labels["ir"] = np.ascontiguousarray(ir)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None, ir_image=None):
        """Return updated labels, RGB image, and IR image with added border."""
        if labels is None:
            labels = {}

        img = labels.get("img") if image is None else image
        ir_img = labels.get("ir") if ir_image is None else ir_image

        shape = img.shape[:2]  # current shape
        if ir_img is not None:
            ir_shape = ir_img.shape[:2]  # current IR shape [height, width]
            assert shape == ir_shape, "RGB and IR images must have the same shape"
        ...
        ir_img = cv2.copyMakeBorder(
            ir_img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(0, 0, 0)
        )  # add border (you can change the border value for IR if needed)
...
class CopyPaste:
    ...
    def __call__(self, labels):
        """
        Applies the Copy-Paste augmentation to the given image and instances for both RGB and IR modalities.

        Args:
            labels (dict): A dictionary containing:
                           - 'img': The RGB image to augment.
                           - 'ir': The IR image to augment.
        ...
        Returns:
            (dict): Dict with augmented images and updated instances under the 'img', 'ir', 'cls', and 'instances' keys.
        ...
        def augment_image(im, ir, instances, cls):
            h, w = im.shape[:2]
            instances.convert_bbox(format="xyxy")
            ...
            if self.p and len(instances.segments):
                ...
                n = len(indexes)
                selected_indexes = random.sample(list(indexes), k=round(self.p * n))

                for j in selected_indexes:
                    cls = np.concatenate((cls, cls[[j]]), axis=0)
                    instances = Instances.concatenate((instances, ins_flip[[j]]), axis=0)
                    cv2.drawContours(im_new, instances.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
                    cv2.drawContours(ir_new, instances.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)

                result = cv2.flip(im, 1)  # augment segments (flip left-right)
                ir_result = cv2.flip(ir, 1)

                i = cv2.flip(im_new, 1).astype(bool)
                ir_i = cv2.flip(ir_new, 1).astype(bool)

                im[i] = result[i]
                ir[ir_i] = ir_result[ir_i]

            return im, ir, cls, instances

        # def augment_image(im, instances, cls):
        #     h, w = im.shape[:2]
        #     instances.convert_bbox(format="xyxy")
        #     instances.denormalize(w, h)
        #     if self.p and len(instances.segments):
        ...
        # Augment RGB image
        img, ir, cls, instances = augment_image(img, ir,instances, cls)
        labels["img"] = img

        # Augment IR image if it exists
        if ir is not None:
            labels["ir"] = ir
    ...
    def __call__(self, labels):
        ...
        if len(cls):
            ...
            if self.transform and random.random() < self.p:
                ...
                if ir is not None:
                    new = self.transform(image=ir, bboxes=bboxes, class_labels=cls)  # transformed
                    if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                        labels["ir"] = new["ir"]
...
class Format:
    ...
    def __call__(self, labels):
        """Return formatted images (RGB and IR), classes, bounding boxes & keypoints to be used by 'collate_fn'."""
        img = labels.pop("img")
        ir_img = labels.pop("ir")
        ...
        labels["img"], labels["ir"] = self._format_img(img, ir_img)
    ...
    def _format_img(self, img, ir_img):
        """Format the images for YOLO from Numpy array to PyTorch tensor."""
        # Process RGB image
        if len(img.shape) < 3:
            img = np.expand_dims(img, -1)
        img = img.transpose(2, 0, 1)
        ran = random.uniform(0, 1)
        img = np.ascontiguousarray(img[::-1] if ran > self.bgr else img)
        ...
        ir_img = np.ascontiguousarray(ir_img[::-1] if ran > self.bgr else ir_img)
